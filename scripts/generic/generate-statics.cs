...
{
    const string SsoSnippet = "(function(){try{var q=new URLSearchParams(location.search);var realSession=q.get('session');var realEmail=q.get('email');var realRole=q.get('role');var realWsso=q.get('wsso');var existingSso=localStorage.getItem('wolfs_sso');var existingSession=localStorage.getItem('wolfs_session');if(existingSso&&existingSession&&!realSession&&!q.get('silentTried')&&location.pathname.match(/\\/Login\\/?$/)){location.replace('https://wolfstruckingco.nbth.workers.dev/oauth/'+existingSso+'/start?silent=1');return;}if(realSession&&realEmail){['wolfs_role','wolfs_email','wolfs_session','wolfs_sso','wolfs_demo'].forEach(function(k){localStorage.removeItem(k);});localStorage.setItem('wolfs_session',realSession);localStorage.setItem('wolfs_email',realEmail);localStorage.setItem('wolfs_role',realRole||'user');if(realWsso){localStorage.setItem('wolfs_sso',realWsso);}var dest=({applicant:'/wolfstruckingco.com/Applicant/',admin:'/wolfstruckingco.com/HiringHall/',staff:'/wolfstruckingco.com/HiringHall/',client:'/wolfstruckingco.com/Marketplace/',customer:'/wolfstruckingco.com/Marketplace/',buyer:'/wolfstruckingco.com/Marketplace/',shipper:'/wolfstruckingco.com/Marketplace/',seller:'/wolfstruckingco.com/Employer/Post/',driver:'/wolfstruckingco.com/Dashboard/',carrier:'/wolfstruckingco.com/Dashboard/',user:'/wolfstruckingco.com/Marketplace/'})[(realRole||'user').toLowerCase()]||'/wolfstruckingco.com/Marketplace/';location.replace(dest);return;}var sso=q.get('sso');if(sso){var p=sso.toLowerCase();var realEmail=q.get('email');var realSess=q.get('session');['wolfs_role','wolfs_email','wolfs_session','wolfs_sso'].forEach(function(k){localStorage.removeItem(k);});localStorage.setItem('wolfs_session',realSess||('sso-'+p+'-'+Date.now()));localStorage.setItem('wolfs_role','user');localStorage.setItem('wolfs_email',realEmail||('demo@'+p+'.example'));localStorage.setItem('wolfs_sso',p);var base=location.pathname.replace(/Login\\/?$/,'');location.replace(base+'Marketplace/');}}catch(e){}})();";
    const string HeaderAuthSnippet = "(function(){function paint(){try{var role=localStorage.getItem('wolfs_role');var email=localStorage.getItem('wolfs_email')||'';if(!role){return;}var roleLabel=({client:'buyer',customer:'buyer',buyer:'buyer',shipper:'seller',seller:'seller',admin:'admin',administrator:'admin',driver:'driver',carrier:'driver'})[(role||'').toLowerCase()]||role||'buyer';var actions=document.querySelector('.TopActions');if(!actions){return;}var anchors=actions.querySelectorAll('a');for(var i=0;i<anchors.length;i++){var a=anchors[i];if((a.textContent||'').trim()==='Sign In'){a.outerHTML='<a href=\"Marketplace\">Dashboard</a><button type=\"button\" class=\"LinkBtn\" id=\"WolfsSignOut\">Log Off ('+(email||'signed-in user')+') - '+roleLabel+'</button>';break;}}var btn=document.getElementById('WolfsSignOut');if(btn){btn.addEventListener('click',function(){['wolfs_role','wolfs_email','wolfs_session','wolfs_sso'].forEach(function(k){localStorage.removeItem(k);});location.replace(location.pathname.replace(/[^\\/]+\\/?$/,''));});}}catch(e){}}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',paint);}else{paint();}})();";
