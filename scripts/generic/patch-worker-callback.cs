...
if (args.Length < 1) { await Console.Error.WriteLineAsync("usage: dotnet run scripts/patch-worker-callback.cs scripts/<config>.cs"); return 1; }
var SpecPath = args[0];
if (!File.Exists(SpecPath)) { await Console.Error.WriteLineAsync($"specific not found: {SpecPath}"); return 2; }

var Strings = new Dictionary<string, string>(StringComparer.Ordinal);
foreach (var Pair in WorkerCallbackPatterns.ConstString().Matches(await File.ReadAllTextAsync(SpecPath)).Select(M => (M.Groups[1].Value, M.Groups[2].Value))) { Strings[Pair.Item1] = Pair.Item2; }
...
var ArrayMatch = WorkerCallbackPatterns.JsBody().Match(Body);
if (!ArrayMatch.Success) { await Console.Error.WriteLineAsync("could not find JsBody() array"); return 2; }

var Lines = WorkerCallbackPatterns.B64Line().Matches(ArrayMatch.Value).Select(M => M.Groups[1].Value).ToList();
if (Lines.Count == 0) { await Console.Error.WriteLineAsync("no base64 lines found"); return 3; }

var B64 = string.Concat(Lines);
var Js = Encoding.UTF8.GetString(Convert.FromBase64String(B64));
if (!Js.Contains(Find, StringComparison.Ordinal))
{
    await Console.Error.WriteLineAsync("anchor not found");
    await Console.Error.WriteLineAsync($"  Find: {Find}");
    return 4;
}
if (Js.Contains(Replace, StringComparison.Ordinal))
{
    await Console.Out.WriteLineAsync("already patched");
    return 0;
}
...
namespace Scripts
{
    internal static partial class WorkerCallbackPatterns
    {
