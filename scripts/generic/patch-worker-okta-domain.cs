...
if (args.Length < 1) { await Console.Error.WriteLineAsync("usage: dotnet run scripts/patch-worker-okta-domain.cs scripts/<config>.cs"); return 1; }
var SpecPath = args[0];
if (!File.Exists(SpecPath)) { await Console.Error.WriteLineAsync($"specific not found: {SpecPath}"); return 2; }

var Strings = new Dictionary<string, string>(StringComparer.Ordinal);
foreach (var Pair in OktaPatchPatterns.ConstString().Matches(await File.ReadAllTextAsync(SpecPath)).Select(M => (M.Groups[1].Value, M.Groups[2].Value))) { Strings[Pair.Item1] = Pair.Item2; }
...
var ArrayMatch = OktaPatchPatterns.JsBody().Match(Body);
if (!ArrayMatch.Success) { await Console.Error.WriteLineAsync("could not find JsBody() array"); return 2; }

var Lines = OktaPatchPatterns.B64Line().Matches(ArrayMatch.Value).Select(M => M.Groups[1].Value).ToList();
if (Lines.Count == 0) { await Console.Error.WriteLineAsync("no base64 lines found"); return 3; }

var B64 = string.Concat(Lines);
var JsBytes = Convert.FromBase64String(B64);
var Js = Encoding.UTF8.GetString(JsBytes);
if (!Js.Contains(Find, StringComparison.Ordinal)) { await Console.Error.WriteLineAsync($"anchor not found: {Find}"); return 4; }
...
namespace Scripts
{
    internal static partial class OktaPatchPatterns
    {
