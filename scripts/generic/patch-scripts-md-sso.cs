...
if (args.Length < 1) { await Console.Error.WriteLineAsync("usage: dotnet run scripts/patch-scripts-md-sso.cs scripts/<config>.cs"); return 1; }
var SpecPath = args[0];
if (!File.Exists(SpecPath)) { await Console.Error.WriteLineAsync($"specific not found: {SpecPath}"); return 2; }

var Strings = PatchScriptsMdSsoPatterns.ConstString().Matches(await File.ReadAllTextAsync(SpecPath))
...
namespace Scripts
{
    internal static partial class PatchScriptsMdSsoPatterns
    {
