...
namespace Scripts
{
    internal static class SceneConfig
    {
        ...
        public const string BeforeShotJs = "() => { try { ['wolfs_role','wolfs_email','wolfs_session','wolfs_sso'].forEach(function(k){localStorage.removeItem(k);}); } catch(e){} location.reload(); return 'cleared'; }";
