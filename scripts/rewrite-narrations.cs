...
// rewrite-narrations.cs — bulk-rewrite specified scene narrations by index.
// Replaces brittle Edit-tool string matching when JSON has ' escapes etc.
//
//   dotnet run scripts/rewrite-narrations.cs -- <scenes.json>

using System.Text.Json.Nodes;

if (args.Length == 0)
{
    Console.Error.WriteLine("usage: dotnet run scripts/rewrite-narrations.cs -- <scenes.json>");
    return 1;
}
var Path = args[0];
if (!File.Exists(Path))
...
foreach (var (Idx, NewNarration) in Updates)
{
    ...
    {
        Console.Error.WriteLine($"  ✗ scene {Idx} out of range");
        continue;
    }
    Arr[Idx - 1]!.AsObject()["narration"] = NewNarration;
    Console.WriteLine($"  ✓ scene {Idx}");
    Touched++;
}

File.WriteAllText(Path, Arr.ToJsonString(new() { WriteIndented = true }));
Console.WriteLine($"rewrote {Touched} narration(s)");
return 0;
