...
public partial class MapPage
{
    ...
    private const string FieldStep1Voice = "step1voice";
    ...
    private const string TrafficClearText = "Clear ahead — auto-rerouting on live traffic.";
    private const string TrafficMediumText = "Moderate congestion ahead.";
    private const string TrafficCongestedText = "Heavy traffic ahead — alternate route active.";
    private const string TrafficClearIcon = "🟢";
    private const string TrafficMediumIcon = "🟡";
    private const string TrafficCongestedIcon = "🔴";
    ...
    private const string OriginD1 = "Hefei";
    private const string DestD1 = "Shanghai";
    private const string OriginD2 = "Port of LA";
    private const string DestD2 = "Phoenix";
    private const string OriginD3 = "Phoenix";
    private const string DestD3 = "Memphis";
    private const string OriginD4 = "Memphis";
    private const string DestD4 = "Wilmington";
    ...
    private string Distance => Latest?[FieldDistance]?.ToString() ?? Dash;

    private string VoiceText => Latest?[FieldStep1Voice]?.ToString() ?? DefaultStep;

    private string DriverShort { get; set; } = Empty;
    ...
    private string TrafficText { get; set; } = TrafficClearText;

    private string TrafficIcon { get; set; } = TrafficClearIcon;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var Rows = Audit
            .Where(R => R is not null && R[FieldKind]?.GetValue<string>() == KindNav)
        ...
        var WorkerName = (await Wolfs.DbAllAsync<JsonObject>(WorkersStore))
            .Where(W => W is not null && string.Equals(W[FieldEmail]?.GetValue<string>() ?? Empty, Email, System.StringComparison.OrdinalIgnoreCase))
            .Select(W => W?[FieldName]?.GetValue<string>() ?? Empty)
            .FirstOrDefault(N => !string.IsNullOrEmpty(N));
        DriverShort = !string.IsNullOrEmpty(WorkerName) ? WorkerName : (Email.Split(AtSign).FirstOrDefault() ?? Empty);

        var (Origin, Destination, Background, Labels) = Email switch
        {
            var E when E.StartsWith(D1Prefix, System.StringComparison.Ordinal) => (OriginD1, DestD1, BackgroundD1, LabelsD1Svg),
            var E when E.StartsWith(D2Prefix, System.StringComparison.Ordinal) => (OriginD2, DestD2, BackgroundD2, LabelsD2Svg),
            var E when E.StartsWith(D3Prefix, System.StringComparison.Ordinal) => (OriginD3, DestD3, BackgroundD3, LabelsD3Svg),
        ...
        if (TrackSubject.Contains(KwDelay, System.StringComparison.Ordinal) || TrackSubject.Contains(KwCongest, System.StringComparison.Ordinal))
        {
            AheadColor = ColorCongested;
            TrafficText = TrafficCongestedText;
            TrafficIcon = TrafficCongestedIcon;
        }
        else if (Progress > Half)
        {
            AheadColor = ColorMedium;
            TrafficText = TrafficMediumText;
            TrafficIcon = TrafficMediumIcon;
        }
    }
