# Wolfs Trucking — 4-Driver International COD Workflow (153 atomic real-user-CRUD scenes)

## Architecture rules

- **No seeding, no demo, no relative captions.** All scene rows are reset at pipeline start. The platform's stores (`applicants`, `listings`, `schedules`, `timesheets`, `charges`, `audit`) grow only by real CRUD actions performed during the workflow.
- **Every scene is an actual real route page** — `/SignUp/`, `/Login/`, `/Applicant/`, `/Interview/`, `/Documents/`, `/HiringHall/`, `/Marketplace/`, `/Employer/Post/`, `/Schedule/`, `/Map/`, `/Itinerary/`, `/Track/`, `/Investors/KPI/`, `/Dispatcher/`. No fake pages.
...
- **No JavaScript authored by hand.** All JS interop in C# code-behind via `IJSRuntime` / `WolfsInteropService`.
...
### Phase 1 — Registration & Login (16)
1. `/SignUp/` — Admin types email admin@wolfstruckingco.com.
2. `/SignUp/` — Admin types password and submits — admin account created.
3. `/Login/` — Admin signs in to platform operator console.
4. `/SignUp/` — Wei types email wei@shanghai-intl.example for Shanghai International Motors.
5. `/SignUp/` — Wei selects payout rail ACH for low-cost listing-fee debit.
6. `/SignUp/` — Wei adds RTP rail for urgent rebalances and submits employer signup.
7. `/SignUp/` — Sam types email sam@buyers.example for Wilmington NC buyer account.
8. `/SignUp/` — Sam selects pay rail RTP / FedNow for the $48,500 COD.
9. `/SignUp/` — Sam submits buyer signup, ready to purchase.
10. `/Login/` — Wei signs in as employer to post listing.
11. `/SignUp/` — Driver 1 Liu Wei (Shanghai CN) registers with payout rail SWIFT wire T-T cross-border China.
12. `/SignUp/` — Driver 2 Pedro Reyes (San Pedro CA) registers with payout rail RTP, FedNow, Visa Direct.
13. `/SignUp/` — Driver 3 Maya + Tom team (Phoenix AZ) registers with payout rail RTP, FedNow, Visa Direct.
14. `/SignUp/` — Driver 4 Jordan Vega (Wilmington NC) registers with payout rail RTP, FedNow, Visa Direct.
15. `/Login/` — Sam signs in to track delivery progress.
16. `/Login/` — Driver 1 signs in to accept first dispatch.
