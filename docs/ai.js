...
(function(){
  ...
  function isLocalHost() {
    return /^(localhost|127\.|\[::1\])/.test(location.hostname) || location.hostname === '';
  }

  async function callProxy(messages, opts) {
    opts = opts || {};
    // Localhost short-circuit: skip the real proxy and signal "fall back to template".
    // Production (cruzlauroiii.github.io) still hits the live worker for real Claude answers.
    if (isLocalHost()) return { ok: false, status: 0, error: 'localhost-dummy' };
    try {
  ...
  window.WolfsAI = {
    ...
    async chat(worker, job, history, message, systemOverride) {
      // Localhost short-circuit: return a scene-ready dummy instead of a template-labelled
      // fallback, so the Applicant intake looks real during video builds and local dev.
      if (isLocalHost()) {
        const m = (message || '').toLowerCase();
        // Intake-style follow-up: applies to the user/applicant flow regardless of stored role.
        if (m.includes('cdl') || m.includes('twic') || m.includes('hazmat') || m.includes('experience') || m.includes('year')) {
          return 'Thanks, Jordan — that\'s a strong foundation to work with. Quick follow-up: are you open to relocating for the right role, or looking to stay based in Wilmington?';
        }
        if (m.includes('earn') || m.includes('work')) {
          return 'You have 1 completed timesheet on file: 8 hours @ $35/hr = $280. No open jobs currently match your badges — check back soon.';
        }
        return 'Thanks for the details — I\'ve noted that on your profile. What else would you like me to flag for admin review?';
      }
