...
// Loads the shared theme chip onto every page that pulls in auth-header.js. The theme
// toggle lives as a standalone script so any page can use it without auth-header if it wants.
(function loadTheme(){
  try {
    if (document.querySelector('script[src*="/theme.js"]')) return;
    var s = document.createElement('script');
    s.src = '/wolfstruckingco.com/theme.js';
    s.async = true;
...
(function(){
  ...
  function apply() {
    ...
    // Deduplicate when the display name is literally the role (e.g. seeded "applicant" / "applicant").
    var showRole = role && name.toLowerCase() !== role.toLowerCase();
    var label = name + (showRole ? ' · ' + role : '') + ' · Sign out';
